...
<Meta title="Guides/Virtualization" />
...
- 📉 **Poor scroll performance** - Browser struggles with large DOM trees
- ⚡ **Janky interactions** - Hover states and clicks become sluggish

Virtualization solves these issues by:
- ✅ **Only rendering visible items** - Typically 10-20 items vs 1000+
- ✅ **Recycling DOM nodes** - Reuses elements as you scroll
- ✅ **Constant memory usage** - Memory doesn't grow with dataset size
- ✅ **60 FPS scrolling** - Smooth performance regardless of list size
...
```tsx
<VirtualGrid
  getItemKey={(item) => item.id} // ✅ Stable ID
  // NOT: getItemKey={(item, index) => index} // ❌ Index changes on reorder
/>
```
