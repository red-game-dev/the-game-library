...
<Meta of={CarouselStories} title="Documentation/Base UI/Carousel" />

# Carousel Component

The Carousel component provides a responsive, touch-enabled slider for displaying multiple items using Embla Carousel. Perfect for image galleries, featured content, testimonials, and product showcases.

## Overview

Built on top of Embla Carousel, this component offers smooth transitions, touch/swipe support, keyboard navigation, and various customization options. It's fully accessible, responsive across all devices, and includes features like infinite scroll, auto-play, and customizable navigation controls.
...
The content to display in the carousel slides. Each child element becomes a slide:
...
Time between slides in milliseconds when autoScroll is enabled.
...
### `dotTheme`
**Type:** `'default' | 'primary' | 'dark' | 'light' | 'neon'`  
**Default:** `'default'`

Theme for the dot indicators:

```tsx
<Carousel dotTheme="neon">
  {slides}
</Carousel>
```
...
function ImageGallery() {
  return (
    <Carousel autoScroll showDots>
      <img src="/img1.jpg" alt="Image 1" className="w-full h-96 object-cover" />
      <img src="/img2.jpg" alt="Image 2" className="w-full h-96 object-cover" />
      <img src="/img3.jpg" alt="Image 3" className="w-full h-96 object-cover" />
    </Carousel>
  );
}
```
...
<Canvas of={CarouselStories.MobileFriendly} />

## Theme Support

The carousel automatically adapts to your application's theme (dark, light, neon, gold) through CSS variables.
