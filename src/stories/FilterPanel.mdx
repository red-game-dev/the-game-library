...
### `disabled`
**Type:** `boolean | { providers?: boolean; types?: boolean; tags?: boolean; special?: boolean; sort?: boolean; viewMode?: boolean }`  
**Default:** `false`

Disable the entire panel or specific sections:
...
<FilterPanel 
  disabled={{
    providers: true,  // Disable provider selection
    types: false,     // Keep types enabled
    tags: true,       // Disable tag selection
    special: false,   // Keep special filters enabled
    sort: false,      // Keep sort enabled
    viewMode: true    // Disable view mode selection
  }}
  filters={filters}
/>
```
...
```tsx
function FilterPanelWithLoading() {
  const [filters, setFilters] = useState(initialFilters);
  const { data: providers, isLoading: providersLoading } = useProviders();
  const { data: tags, isLoading: tagsLoading } = useTags();
  const isLoading = providersLoading || tagsLoading;
...
function ConditionalFilters() {
  ...
  // Disable certain sections based on user role or context
  const disabledSections = {
    providers: userRole === 'guest',  // Guests can't filter by provider
    types: false,                      // Always enabled
    tags: filters.types.length === 0,  // Enable tags only when type selected
    special: userRole === 'guest',     // Guests can't use special filters
    sort: false,                       // Always enabled
    viewMode: false                    // Always enabled
  };
...
<Canvas of={FilterPanelStories.MobileDrawer} />
