...
The SearchBar component provides advanced search functionality with debouncing, loading states, search type toggle, and keyboard shortcuts. It's optimized for searching through large game libraries with instant feedback across multiple content types.

## Overview

The SearchBar is a feature-rich search component built on top of the base Input component. It includes debounced search, clear functionality, loading indicators, search type toggle for multi-context searching (games, providers, tags), and keyboard shortcuts for enhanced user experience.
...
### `onSearch`
**Type:** `(query: string, searchType?: SearchType) => void`  
**Default:** `undefined`

Callback fired after debounce delay with optional search type:

```tsx
<SearchBar 
  onSearch={(query, searchType) => {
    // Perform search with debounced query and type
    if (searchType === 'games') {
      searchGames(query);
...
Enable search type toggle for multi-context searching:
...
### `defaultSearchType`
**Type:** `'all' | 'games' | 'providers' | 'tags'`  
**Default:** `'all'`

Default search type when toggle is enabled:
...
The SearchBar supports multi-context searching with a visual toggle:

<Canvas of={SearchBarStories.WithTypeDropdown} />

### Available Search Types

- **All** - Search across all content types
- **Games** - Search game titles, descriptions, and metadata
- **Providers** - Search game developers and publishers
- **Tags** - Search game categories and tags

### Implementation Example

```tsx
function MultiContextSearch() {
  const handleSearch = (query: string, searchType?: SearchType) => {
    switch(searchType) {
      case 'games':
        searchGamesAPI(query);
        break;
      case 'providers':
        searchProvidersAPI(query);
        break;
      case 'tags':
        searchTagsAPI(query);
        break;
